...
* Function list: 
*	mainParser(), printParserError(), getParserFileSize(),
*	displayParser(), callGarbageCollector()
************************************************************
*/
...
/* External objects */
extern mouse_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern mouse_int line; /* source code line number - defined in scanner.c */
...
extern mouse_None startParser(mouse_None);
extern mouse_int startScanner(BufferPointer sc_buf);

static mouse_None printParserError(mouse_str fmt, ...);
static mouse_None displayParser(BufferPointer ptrBuffer);
static mouse_long getParserFileSize(mouse_str fname);
static mouse_None callGarbageCollector(mouse_None);
...
mouse_None printParserError(mouse_str fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(mouse_None)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
mouse_long getParserFileSize(mouse_str fname) {
	FILE* input;
	mouse_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
mouse_None displayParser(BufferPointer ptrBuffer) {
...
mouse_None callGarbageCollector(mouse_None) {
